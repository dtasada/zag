...
pub fn mapGenerics(alloc: std.mem.Allocator, map: std.StringHashMap(ast.Expression), node: anytype) (std.mem.Allocator.Error || error{UnhandledGenericMappingTypeSubstitution})!void {
    ...
    if (T == []const u8 or T == usize or T == bool or T == f64 or T == ast.BinaryOperator or T == ast.AssignmentOperator or T == ast.PrefixOperator) return;
        ...
        if (node.* == .symbol) {
            if (map.get(node.symbol.inner)) |expr| {
                ...
                if (expr == .ident) {
                    ...
                    return error.UnhandledGenericMappingTypeSubstitution;
                }
            }
        }
    }
    ...
    switch (t_info) {
        ...
        .optional => {
            if (node.*) |*payload| try mapGenerics(alloc, map, payload);
        },
        else => {},
    }
}
